...
const PixManager = {
  ...
  gerarPayload(valor) {
    ...
    let payload =
      this._tlv("00", "01") +         // Format Indicator
      mai +                             // Merchant Account
      this._tlv("52", "0000") +        // MCC
      this._tlv("53", "986");          // Currency BRL

    // Valor (opcional)
    if (valor && valor > 0) {
      payload += this._tlv("54", valor.toFixed(2));
    }

    payload +=
      this._tlv("58", "BR") +          // Country
      this._tlv("59", nome) +          // Merchant Name
      this._tlv("60", cidade) +        // City
      ad;                               // Additional Data
  ...
  async gerarQRBase64(payload) {
    ...
    qr.addData(payload);
    qr.make();
    // Gera imagem base64 (cellSize=8, margin=4)
    return qr.createDataURL(8, 4);
  },

  // ===== MODAL =====
  async mostrar(valorVenda) {
    const cfg = this.getConfig();
    if (!cfg.chave) {
      this._mostrarConfig();
      return;
    ...
    const html = `
      <div style="text-align:center">
        ...
        <p style="font-size:22px;font-weight:800;color:#10b981;margin:12px 0">${fmt(valorVenda)}</p>
        ...
        <button onclick="PixManager._mostrarConfig()" style="margin-top:10px;background:none;border:none;color:#4f7cff;cursor:pointer;font-size:12px;font-weight:600">⚙️ Alterar configuração PIX</button>
      </div>`;

    Modal.show("📱 Pagamento via PIX", html, [
      { label: "Fechar", cls: "ghost", onClick: () => Modal.close() }
    ]);
  },
